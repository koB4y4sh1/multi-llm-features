...
This folder is numbered in a beginner-friendly learning order:

1. `01_text.py`: Start with the smallest text generation example.
2. `02_conversation.py`: Learn how multi-turn chat keeps context.
3. `03_multi_modal.py`: Add image input after basic text usage is clear.
4. `04_file_input.py`: Move from images to larger file inputs such as video.
5. `05_token.py`: Check token usage before moving into more advanced features.
6. `06_code_interpreter.py`: Try tool-assisted generation after the basics.
7. `07_embedding.py`: Learn embeddings after prompt/chat concepts are familiar.
8. `08_structured_output.py` - `14_live.py`: Advanced or application-oriented topics.
